...
@Test func saveAndLoadLayout() throws {
    ...
    let layout = Layout(
        ...
        zones: [
            Zone(position: .leftHalf, appBundleID: "com.apple.Terminal", path: "/tmp", displayIndex: 0),
        ]
    )
...
@Test func deleteLayout() throws {
    ...
    let store = LayoutStore(directory: tempDir)
    let layout = Layout(name: "ToDelete", gridTemplate: "maximize", zones: [])
...
@Test func updateLayout() throws {
    ...
    let store = LayoutStore(directory: tempDir)
    var layout = Layout(name: "Original", gridTemplate: "maximize", zones: [])
