...
struct LayoutEditorView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Layout").font(.system(size: 12, weight: .medium)).foregroundColor(.secondary)
                    ...
                    if !zones.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("Zones").font(.system(size: 12, weight: .medium)).foregroundColor(.secondary)
    ...
    private func saveLayout() {
        ...
        var layout = Layout(name: name, gridTemplate: template.id, zones: zones)
